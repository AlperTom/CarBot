...
export async function initializeRedis() {
  ...
  try {
    // Skip Redis in production if not configured to avoid build errors
    if (process.env.NODE_ENV === 'production' && !process.env.REDIS_URL) {
      console.log('Redis not configured for production, using in-memory storage');
      return null;
    }
    ...
    redisClient.on('error', (err) => {
      ...
      return null; // Don't throw, just disable Redis
    });
    ...
    console.warn('Failed to initialize Redis, using fallback:', error.message);
...
export function getJWTSecret() {
  ...
  if (!secret || weakSecrets.includes(secret) || secret.length < 32) {
    if (process.env.NODE_ENV === 'production') {
      console.error('SECURITY ERROR: Weak or missing JWT secret in production - generating secure fallback');
      // In production, generate and log a secure secret (admin should set proper env var)
