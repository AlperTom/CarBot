...
// JWT Configuration
import { getJWTSecret, sessionManager } from './security.js'

const JWT_SECRET = getJWTSecret()
const JWT_EXPIRES_IN = process.env.JWT_EXPIRES_IN || '24h'
const REFRESH_TOKEN_EXPIRES_IN = process.env.REFRESH_TOKEN_EXPIRES_IN || '7d'

// Use secure session manager instead of in-memory storage
// Temporary in-memory storage for development (replace with Redis in production)
...
export function generateTokens(user, workshop = null, role = 'customer') {
  ...
  // Store refresh token securely (async operation moved to caller)
  // sessionManager.storeRefreshToken(refreshPayload.jti, user.id, 7 * 24 * 60 * 60) // 7 days
