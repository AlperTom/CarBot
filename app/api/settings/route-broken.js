...
export async function GET(request) {
  try {
    ...
    // Try to fetch from database with proper column name
    console.log('🔍 Attempting to fetch workshop data for user email:', user.email)
    ...
    try {
      ...
      console.log('Workshop table sample:', workshopData)
      
      // Now try to find workshop by owner_email (correct column name)
      const { data: workshop, error: queryError } = await supabase
        .from('workshops')
        .select('*')
        .eq('owner_email', user.email)
        .single()
        
      console.log('Workshop query result:', { workshop, queryError })

      if (!queryError && workshop) {
          return NextResponse.json({
